...
let MoviesService = MoviesService_1 = class MoviesService {
    constructor() {
        ...
        this.genres = new Map();
        this.state = {
            nflix: { movies: new Map(), tmdbIdIndex: new Map(), titleIndex: new Map(), genreIndex: new Map(), categories: [], realRecentlyAddedTmdbIds: new Set(), realLeavingSoonTmdbIds: new Set(), lastRefreshAttemptAt: 0, refreshInFlight: null, searchCache: new Map() },
            nprime: { movies: new Map(), tmdbIdIndex: new Map(), titleIndex: new Map(), genreIndex: new Map(), categories: [], realRecentlyAddedTmdbIds: new Set(), realLeavingSoonTmdbIds: new Set(), lastRefreshAttemptAt: 0, refreshInFlight: null, searchCache: new Map() },
            hotstar: { movies: new Map(), tmdbIdIndex: new Map(), titleIndex: new Map(), genreIndex: new Map(), categories: [], realRecentlyAddedTmdbIds: new Set(), realLeavingSoonTmdbIds: new Set(), lastRefreshAttemptAt: 0, refreshInFlight: null, searchCache: new Map() },
        };
        this.providerMap = {
            'nflix': '8',
            'nprime': '9',
    ...
    async onModuleInit() {
        ...
        try {
            ...
            if (providers && providers.results) {
                const nflix = providers.results.find((p) => p.provider_name.toLowerCase().includes('netflix'));
                const nprime = providers.results.find((p) => p.provider_name.toLowerCase().includes('amazon prime video'));
                ...
                if (nflix)
                    this.providerMap['nflix'] = String(nflix.provider_id);
                if (nprime)
                    this.providerMap['nprime'] = String(nprime.provider_id);
                if (hotstar)
    ...
    toMovie(item, mediaType) {
        ...
        const rawSources = isTV ? [
            { name: 'VidLink', url: `https://vidlink.pro/tv/${tmdbIdStr}/1/1?${vl}`, type: 'stream' },
            { name: 'AutoEmbed', url: `https://autoembed.co/tv/tmdb/${tmdbIdStr}-1-1`, type: 'stream' },
            { name: 'VidSrc.pro', url: `https://vidsrc.pro/embed/tv/${tmdbIdStr}/1/1`, type: 'stream' },
            { name: 'VidSrc.cc', url: `https://vidsrc.cc/v2/embed/tv/${tmdbIdStr}/1/1`, type: 'stream' },
            { name: 'Embed.su', url: `https://embed.su/embed/tv/${tmdbIdStr}/1/1`, type: 'stream' },
            { name: '2Embed (Cineby)', url: `https://www.2embed.cc/embed/${tmdbIdStr}/1/1`, type: 'stream' },
            { name: 'VidSrc', url: `https://vidsrc.pm/embed/tv/${tmdbIdStr}/1/1`, type: 'stream' },
            { name: 'Mapple (4KHD)', url: `https://www.2embed.cc/embed/${tmdbIdStr}/1/1#mapple`, type: 'stream' },
            { name: 'Main', url: `https://multiembed.mov/directstream.php?video_id=${tmdbIdStr}&tmdb=1&s=1&e=1`, type: 'stream' },
            { name: 'Prime', url: `https://primestream.io/embed/tv/${tmdbIdStr}/1/1`, type: 'stream' }
        ] : [
            { name: 'VidLink', url: `https://vidlink.pro/movie/${tmdbIdStr}?${vl}`, type: 'stream' },
            { name: 'AutoEmbed', url: `https://autoembed.co/movie/tmdb/${tmdbIdStr}`, type: 'stream' },
            { name: 'VidSrc.pro', url: `https://vidsrc.pro/embed/movie/${tmdbIdStr}`, type: 'stream' },
            { name: 'VidSrc.cc', url: `https://vidsrc.cc/v2/embed/movie/${tmdbIdStr}`, type: 'stream' },
            { name: 'Embed.su', url: `https://embed.su/embed/movie/${tmdbIdStr}`, type: 'stream' },
            { name: '2Embed (Cineby)', url: `https://www.2embed.cc/embed/${tmdbIdStr}`, type: 'stream' },
            { name: 'VidSrc', url: `https://vidsrc.pm/embed/movie/${tmdbIdStr}`, type: 'stream' },
            { name: 'Mapple (4KHD)', url: `https://www.2embed.cc/embed/${tmdbIdStr}#mapple`, type: 'stream' },
            { name: 'Main', url: `https://multiembed.mov/directstream.php?video_id=${tmdbIdStr}&tmdb=1`, type: 'stream' },
            { name: 'Prime', url: `https://primestream.io/embed/movie/${tmdbIdStr}`, type: 'stream' },
            { name: 'Torrent Web (Multi-Audio)', url: `webtor:${encodeURIComponent(item.title || item.name)}|${(item.release_date || item.first_air_date || '').substring(0, 4)}`, type: 'stream' }
        ];
        ...
        return {
            ...
            isSeries: isTV,
            isAnime: isAnime,
            genres: (item.genre_ids || []).map((id) => this.genres.get(id)).filter((name) => Boolean(name)),
            cast: [],
            director: '',
            tags: [],
    ...
    async refreshCatalog(platform = "nflix") {
        const state = this.state[platform];
    ...
    buildDynamicRails(platform) {
        ...
        const regionalRails = regionalLanguages.flatMap(([code, name]) => [
            { id: `${code}-movies`, name: `Popular ${name} Movies`, mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_original_language=${code}&sort_by=popularity.desc`, pages: 1 },
            { id: `${code}-series`, name: `Popular ${name} Series`, mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_original_language=${code}&sort_by=popularity.desc`, pages: 1 },
        ]);
        const getRandomName = (names) => names[Math.floor(Math.random() * names.length)];
        const curatedRails = [
            { id: 'scifi-hits', name: getRandomName(['Sci-Fi Mindbenders', 'Imaginative Sci-Fi', 'Out of This World']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=878&sort_by=vote_average.desc&vote_count.gte=500` },
            { id: 'horror-nights', name: getRandomName(['Horror & Thrills', 'Chilling Horror Movies', 'Ominous Thrillers', 'Scary Movies']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=27,53&sort_by=popularity.desc` },
            { id: 'family-time', name: getRandomName(['Family Favorites', 'Movies for the Whole Family', 'Kids & Family']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=10751&sort_by=popularity.desc` },
            { id: 'documentary', name: getRandomName(['Documentary Features', 'Critically Acclaimed Documentaries', 'Real Life Stories']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=99&sort_by=popularity.desc` },
            { id: 'comedy-gold', name: getRandomName(['Comedy Gold', 'Feel-Good Comedies', 'Laugh-Out-Loud Movies']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=35&sort_by=popularity.desc` },
            { id: 'action-packed', name: getRandomName(['High Octane Action', 'Action & Adventure', 'Explosive Action Movies']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=28&sort_by=popularity.desc` },
            { id: 'romance-picks', name: getRandomName(['Romantic Dramas', 'Heartfelt Movies', 'Romance']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=10749&sort_by=popularity.desc` },
            { id: 'thriller-tv', name: getRandomName(['Gripping TV Thrillers', 'Suspenseful TV Shows', 'Crime Thrillers']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_genres=80,53&sort_by=popularity.desc` },
            { id: 'crime-series', name: getRandomName(['Crime & Investigation', 'True Crime Inspired', 'Gritty Crime TV Shows']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_genres=80&sort_by=popularity.desc` },
            { id: 'mystery-box', name: getRandomName(['Mystery & Suspense', 'Whodunit TV Shows', 'Mind-Bending Mysteries']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_genres=9648&sort_by=popularity.desc` },
            { id: 'award-winners', name: getRandomName(['Award-Winning Cinema', 'Critically Acclaimed Movies', 'Oscar Winners']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&vote_average.gte=8&vote_count.gte=1000` },
            { id: 'classic-rewind', name: getRandomName(['Classic Rewind', 'Nostalgic Movies', 'Throwback Movies']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&release_date.lte=1990-12-31&sort_by=popularity.desc` },
            { id: 'indie-gems', name: getRandomName(['Indie & Art House', 'Independent Movies', 'Critically Acclaimed Indie']), mediaType: 'movie', path: `discover/movie?${baseDiscoverMovie}&with_genres=18&vote_average.gte=7&vote_count.gte=200` },
            { id: 'k-drama', name: getRandomName(['K-Drama Hits', 'Korean TV Shows', 'Binge-Worthy K-Dramas']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_original_language=ko&sort_by=popularity.desc` },
            { id: 'british-tv', name: getRandomName(['British TV Dramas', 'Acclaimed British Shows', 'Made in the UK']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_original_language=en&origin_country=GB&sort_by=popularity.desc` },
            { id: 'fantasy-realms', name: getRandomName(['Fantasy Worlds', 'Epic Fantasy Series', 'Magical TV']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_genres=10765&sort_by=popularity.desc` },
            { id: 'animation-tv', name: getRandomName(['Animated Series', 'Adult Animation', 'Toons for Everyone']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_genres=16&sort_by=popularity.desc` },
            { id: 'reality-tv', name: getRandomName(['Reality TV', 'Unscripted TV', 'Binge-Worthy Reality']), mediaType: 'tv', path: `discover/tv?${baseDiscoverTv}&with_genres=10764&sort_by=popularity.desc` },
        ];
    ...
    async ensureCatalog(platform = 'nflix') {
        this.ensureConfigured();
        const state = this.state[platform];
    ...
    async getAllMovies(platform = 'nflix') {
    ...
    async getTop10Movies(platform = 'nflix') {
        await this.ensureCatalog(platform);
        const allMovies = this.state[platform].categories.flatMap(c => c.movies);
        const uniqueMap = new Map();
        for (const m of allMovies) {
    ...
    async getFeaturedMovie(platform = 'nflix') {
        await this.ensureCatalog(platform);
    ...
    async getCategories(platform = 'nflix') {
        await this.ensureCatalog(platform);
    ...
    async getMovieById(id, platform = 'nflix') {
        await this.ensureCatalog(platform);
        ...
        if (!movie.cast?.length || !movie.videoUrl || !movie.logoUrl || !movie.trailerUrl || movie.seasonsCount === undefined) {
            try {
                ...
                const tagsList = rawKeywords.slice(0, 5).map((k) => {
                    return k.name.split(' ').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                });
                if (tagsList.length)
                    movie.tags = tagsList;
                const fullCast = (details.credits?.cast || []).slice(0, 10).map((c) => ({
                    id: c.id,
                    name: c.name,
                ...
                const director = details.credits?.crew?.find((c) => c.job === 'Director')?.name || details.credits?.crew?.find((c) => c.job === 'Executive Producer')?.name;
                if (director)
        ...
        const allPlatforms = ['nflix', 'nprime', 'hotstar'];
        const platformLabel = { nflix: 'Netflix', nprime: 'Prime Video', hotstar: 'Hotstar' };
    ...
    async getSeasonEpisodes(id, seasonNumber, platform = 'nflix') {
        await this.ensureCatalog(platform);
        let movie = this.state[platform].movies.get(id);
        if (!movie) {
            const internalId = this.state[platform].tmdbIdIndex.get(id);
        ...
        try {
            ...
            const episodes = rawEpisodes.map((ep) => {
                ...
                return {
                    ...
                    sources: [
                        { name: 'VidLink', url: vidLinkUrl, type: 'stream' },
                        { name: 'AutoEmbed', url: `https://autoembed.co/tv/tmdb/${movie.tmdbId}/${s}/${e}`, type: 'stream' },
                        { name: 'VidSrc.pro', url: `https://vidsrc.pro/embed/tv/${movie.tmdbId}/${s}/${e}`, type: 'stream' },
                        { name: 'VidSrc.cc', url: `https://vidsrc.cc/v2/embed/tv/${movie.tmdbId}/${s}/${e}`, type: 'stream' },
                        { name: 'Embed.su', url: `https://embed.su/embed/tv/${movie.tmdbId}/${s}/${e}`, type: 'stream' },
                        { name: '2Embed (Cineby)', url: `https://www.2embed.cc/embed/${movie.tmdbId}/${s}/${e}`, type: 'stream' },
                        { name: 'VidSrc', url: `https://vidsrc.pm/embed/tv/${movie.tmdbId}/${s}/${e}`, type: 'stream' },
                        { name: 'Mapple (4KHD)', url: `https://www.2embed.cc/embed/${movie.tmdbId}/${s}/${e}#mapple`, type: 'stream' },
                        { name: 'Main', url: `https://multiembed.mov/directstream.php?video_id=${movie.tmdbId}&tmdb=1&s=${s}&e=${e}`, type: 'stream' },
                        { name: 'Prime', url: `https://primestream.io/embed/tv/${movie.tmdbId}/${s}/${e}`, type: 'stream' }
    ...
    async searchMovies(query, genre, platform = 'nflix') {
        this.ensureConfigured();
        ...
        try {
            ...
            let resultsWithScores = Array.from(combinedMoviesMap.values()).map(m => {
                ...
                if (t === normalized)
                    ...
                    score += 10;
                if (m.originalTitle && m.originalTitle.toLowerCase().includes(normalized))
                    score += 5;
                if (m.genres && m.genres.some(g => g.toLowerCase().includes(normalized)))
                    score += 5;
                if (m.tags && m.tags.some(g => g.toLowerCase().includes(normalized)))
                    score += 3;
                const castStr = m.cast ? m.cast.map((c) => typeof c === 'string' ? c : c.name).join(' ').toLowerCase() : '';
            ...
            if (!hasStrongLocalMatch && normalized.length > 2) {
                ...
                try {
                    ...
                    if (tmdbSearch.results && tmdbSearch.results.length > 0) {
                        const topHits = tmdbSearch.results.slice(0, 5).filter((m) => m.media_type === 'movie' || m.media_type === 'tv');
                        const liveResults = [];
                        for (const hit of topHits) {
                            ...
                            const availableOn = [];
                            if (allProviderIds.includes(this.providerMap['nflix']))
                                availableOn.push('Netflix');
                            if (allProviderIds.includes(this.providerMap['nprime']))
    ...
    filterGenre(titles, genre) {
        return genre && genre !== 'All' ? titles.filter((item) => Array.isArray(item.genres) && item.genres.includes(genre)) : titles;
    }
    async getSimilarMovies(id, platform = 'nflix') {
        await this.ensureCatalog(platform);
        ...
        if (mainKeyword.length > 3) {
            const titleMatches = allMovies.filter(m => m.id !== current.id &&
        ...
        try {
                ...
                try {
                    ...
                    if (franchiseSearch && franchiseSearch.results) {
                        const franchiseHits = franchiseSearch.results
                            .filter((m) => (m.media_type === 'movie' || m.media_type === 'tv') && String(m.id) !== current.tmdbId)
                            .slice(0, 5)
            ...
            if (recs && recs.results) {
                similar.push(...recs.results.slice(0, 8).map((item) => this.toMovie(item, type)));
            }
            const sim = await this.tmdb(`${type}/${current.tmdbId}/similar`);
            if (sim && sim.results) {
                similar.push(...sim.results.slice(0, 8).map((item) => this.toMovie(item, type)));
            }
        }
        catch (e) {
            this.logger.warn(`Failed to fetch true similar movies for ${id}: ${e}`);
        }
        const genreMatches = allMovies.filter(item => item.id !== current.id && item.genres.some((genre) => current.genres.includes(genre))).slice(0, 10);
    ...
    async getRecommendations(id, platform = 'nflix') {
        await this.ensureCatalog(platform);
        let source;
        try {
        ...
        const scored = allMovies
            ...
            const sourceCastNames = source.cast.map((c) => typeof c === 'string' ? c.toLowerCase() : c.name?.toLowerCase() || '').filter(Boolean);
            const targetCastNames = m.cast.map((c) => typeof c === 'string' ? c.toLowerCase() : c.name?.toLowerCase() || '').filter(Boolean);
    ...
    async getIntroTimings(id, season, episode, platform = 'nflix') {
        try {
            await this.ensureCatalog(platform);
            ...
            return {
                ...
                startSeconds: 0,
                endSeconds: 0,
                note: 'Intro timings require server-side chapter detection',
            };
        }
        catch {
            return { hasIntro: false, startSeconds: 0, endSeconds: 0 };
    ...
    async getMagnetLink(title, year) {
        try {
